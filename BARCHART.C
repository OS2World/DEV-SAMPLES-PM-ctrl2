...
BOOL EXPENTRY SetDlgItemLong(HWND hwndDlg, ULONG idItem, ULONG ulValue, BOOL fSigned)
...
BOOL EXPENTRY QueryDlgItemLong(HWND hwndDlg, ULONG idItem, PLONG pResult, BOOL fSigned)
...
VOID GetBarChartData(HWND hWnd, PBARCHARTCDATA pbccd, PUSERSTYLE pust)
   ...
   case WM_PAINT :
       ...
       if ( pbc->fDraw )
           ...
           if ( pbc->cItems )
               for ( i = 0; i < pbc->cItems; i++ )
                   {
                   WinFillRect(hPS, &pbc->pbci[i].rcl, RGB_RED);
